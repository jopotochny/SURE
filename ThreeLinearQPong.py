...
def main():
    ...
    def plot_score():
        ...
        if len(score_t) > scoreSaveLength:  # save plot every 10 episodes
            ...
            plt.savefig(PATH+str(learning_rate)+"threelinearscoreplt.png")
            plt.close(fig)
        if is_ipython:
        ...
        for t in count():
            ...
            if done:
                ...
                save_checkpoint({'episodes': episode_durations,
                                     ...
                                     'optimizer': optimizer.state_dict(), 'scores': scores, 'steps_done':steps_done}, PATH + "threelinearcheckpoint.pt")
                p.reset_game()
                break
