...
def main():
    ...
    def plot_score():
        ...
        if len(score_t) > scoreSaveLength:  # save plot every 10 episodes
            ...
            plt.savefig(PATH+"TFscoreplt.png")
            plt.close(fig)
        if is_ipython:
    ...
    def plotLoss():
        ...
        plt.savefig(PATH + "TFlossplt.png")
        plt.close(plot)
    ...
    for i_episode in range(episodes_left):
        ...
        for t in count():
            ...
            episode_memory.append(Transition(observation, action, reward))
            ...
            if done:
                ...
                if(flag == True):
                    save_checkpoint({'episodes': episode_durations,
                                     ...
                                     'losses': losses}, PATH + "TFcheckpoint1.pt")
                    flag = not flag
                else:
                    save_checkpoint({'episodes': episode_durations,
                                     'state_dict': policy_net.state_dict(),
                                     'target_dict': target_net.state_dict(),
                                     'optimizer': optimizer.state_dict(), 'scores': scores, 'steps_done': steps_done,
                                     'losses': losses}, PATH + "TFcheckpoint2.pt")
                    flag = not flag
                score = 0
