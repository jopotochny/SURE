...
def main():
    ...
    for i_episode in range(num_episodes):
        ...
        window.lock()
        last_screen = np.ascontiguousarray(np.flip(pygame.surfarray.pixels3d(window).transpose(2, 1, 0), axis=0).copy(),
                                           dtype=np.float32)
        window.unlock()
        ...
        for t in count():
            ...
            fps.tick(60)
            window.lock()
            current_screen = np.ascontiguousarray(
                np.flip(pygame.surfarray.pixels3d(window).transpose(2, 1, 0), axis=0).copy(), dtype=np.float32)
            window.unlock()
